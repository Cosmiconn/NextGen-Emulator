import re

XOR_TABLE = bytes([
])
assert len(XOR_TABLE) == 499

def crypt(data, xorpos):
    out = bytearray(data)
    pos = xorpos
    for i in range(len(out)):
        out[i] ^= XOR_TABLE[pos]
        pos = (pos + 1) % 499
    return bytes(out), pos

def parse_frames(buf):
    """Zerlegt einen TCP-Payload-Strom in (headerlen, packetlen, encrypted_payload, rest) Frames."""
    frames = []
    i = 0
    while i < len(buf):
        if buf[i] == 0:
            if i + 3 > len(buf): break
            plen = int.from_bytes(buf[i+1:i+3], 'little')
            hlen = 3
        else:
            plen = buf[i]
            hlen = 1
        if i + hlen + plen > len(buf):
            break
        payload = buf[i+hlen:i+hlen+plen]
        frames.append((hlen, plen, payload))
        i += hlen + plen
    return frames, buf[i:]

if __name__ == "__main__":
    print("XOR_TABLE geladen, Laenge:", len(XOR_TABLE))

def unpack_opcode(payload):
    """Liest die ersten 2 Bytes als Little-Endian ushort und entpackt sie
    gemaess Packet(byte,byte)-Konvention: Header = opcode>>10, Type = opcode&1023."""
    if len(payload) < 2:
        return None, None, 0
    opcode = payload[0] | (payload[1] << 8)
    header = opcode >> 10
    ptype = opcode & 1023
    return header, ptype, opcode
