...
def report_low_item_ids():
    if not ITEM_SQL.is_file():
        ...
        return
    text = ITEM_SQL.read_text(encoding='utf-8', errors='replace')
    found = {}
    for item_id in range(4):
        pat = re.compile(r'(?mi)^.*(?:VALUES\s*)?\(\s*' + str(item_id) + r'\s*,.*    stage_names = ('Start', 'Action', 'Finish')
    unresolved = []
    cross = []
    undefined = []
    ambiguous = []
...
if __name__ == '__main__':
    raise SystemExit(main())
        ...
        m = pat.search(text)
        found[item_id] = m.group(0)[:240] if m else None
    print('REVIEW: low ItemID rows in data_iteminfo.sql:')
    for item_id in range(4):
        print(f'  {item_id}: ' + (found[item_id] if found[item_id] else '<not found>'))
