...
namespace NextGen.KingdomQuestTool
{
    internal static class Program
    {
            ...
            using (var writer = new StreamWriter(output, false, new UTF8Encoding(false)))
            {
                ...
                foreach (string path in inputs.OrderBy(p => p, StringComparer.OrdinalIgnoreCase))
                {
                    ...
                    using (var table = new SHNFile(path))
                    {
                        ...
                        WriteTable(writer, table, sourceName, sha256);
                    }
                }
            }
        ...
        private static void WriteManifestSchema(TextWriter writer)
        {
            writer.WriteLine("DROP TABLE IF EXISTS `data_kq_source_manifest`;");
            writer.WriteLine("CREATE TABLE `data_kq_source_manifest` (");
            writer.WriteLine("  `SourceName` VARCHAR(64) NOT NULL,");
            writer.WriteLine("  `Sha256` CHAR(64) NOT NULL,");
            writer.WriteLine("  `RecordCount` INT UNSIGNED NOT NULL,");
            writer.WriteLine("  `ColumnCount` INT UNSIGNED NOT NULL");
            writer.WriteLine(") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");
            writer.WriteLine();
            writer.WriteLine("DROP TABLE IF EXISTS `data_kq_source_columns`;");
            writer.WriteLine("CREATE TABLE `data_kq_source_columns` (");
            writer.WriteLine("  `SourceName` VARCHAR(64) NOT NULL,");
            writer.WriteLine("  `Ordinal` INT UNSIGNED NOT NULL,");
            writer.WriteLine("  `ColumnName` VARCHAR(128) NOT NULL,");
            writer.WriteLine("  `TypeByte` INT UNSIGNED NOT NULL,");
            writer.WriteLine("  `Length` INT UNSIGNED NOT NULL");
            writer.WriteLine(") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");
            writer.WriteLine();
        }

        private static void WriteManifestRows(TextWriter writer, SHNFile table,
            string sourceName, string sha256)
        {
            writer.Write("INSERT INTO `data_kq_source_manifest` VALUES (");
            WriteSqlString(writer, sourceName);
            writer.Write(", ");
            WriteSqlString(writer, sha256);
            writer.Write(", {0}, {1});", table.Rows.Count, table.Columns.Count);
            writer.WriteLine();

            if (table.Columns.Count != 0)
            {
                writer.WriteLine("INSERT INTO `data_kq_source_columns` VALUES");
                for (int i = 0; i < table.Columns.Count; i++)
                {
                    ...
                    WriteSqlString(writer, sourceName);
                    writer.Write(", {0}, ", i);
                    WriteSqlString(writer, column.ColumnName);
                    writer.Write(", {0}, {1})", column.TypeByte, column.Length);
        ...
        private static void WriteSqlString(TextWriter writer, string value)
        {
            writer.Write("'");
