...
namespace NextGen.Zone.Data
{
    internal static class QuestRuntime
    {
        ...
        public static bool Complete(ZoneCharacter c,uint questId,uint selectedIndex=0,bool selectionProvided=false){if(!IsComplete(c,questId))return false;if(!selectionProvided){using(DatabaseClient rewardDb=Program.DatabaseManager.GetClient())if(NeedsRewardSelection(rewardDb,questId))return false;}if(selectionProvided&&!HasSelectableRewardIndex(c,questId,selectedIndex))return false;try{using(DatabaseClient db=Program.DatabaseManager.GetClient()){DataTable state=db.ReadDataTable("SELECT Status FROM character_quest_state WHERE CharID=@c AND QuestID=@q",new MySqlParameter("@c",c.ID),new MySqlParameter("@q",questId));if(state==null||state.Rows.Count==0||Convert.ToByte(state.Rows[0]["Status"])!=PqsInProgress)return false;byte completionStatus=IsRepeatable(db,questId)?PqsRepeat:PqsDone;db.ExecuteQuery("UPDATE character_quest_state SET Status=@s WHERE CharID=@c AND QuestID=@q",new MySqlParameter("@s",completionStatus),new MySqlParameter("@c",c.ID),new MySqlParameter("@q",questId));ApplyRewards(db,c,questId,selectedIndex);return true;}}catch(Exception ex){Log.WriteLine(LogLevel.Warn,"Quest completion failed {0}: {1}",questId,ex.Message);return false;}}
