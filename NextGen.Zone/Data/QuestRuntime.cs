    ...
    internal static class QuestRuntime
    {
        ...
        public static void RecordMobKill(ZoneCharacter character, ushort mobId)
        {
            ...
            try
            {
                using (DatabaseClient db = Program.DatabaseManager.GetClient())
                {
                    ...
                    {
                        uint q = Convert.ToUInt32(row["QuestID"]); byte slot = Convert.ToByte(row["Slot"]); uint amount = Convert.ToUInt32(row["Amount"]); uint oldValue = Convert.ToUInt32(row["Progress"]);
                        uint value = Math.Min(amount, oldValue + 1);
                        db.ExecuteQuery("INSERT INTO character_quest_progress (CharID,QuestID,Slot,Progress) VALUES (@c,@q,@slot,@p) ON DUPLICATE KEY UPDATE Progress=@p",
        ...
        public static bool IsComplete(ZoneCharacter c, uint questId)
        {
            try
            {
                using (DatabaseClient db = Program.DatabaseManager.GetClient())
                {
                    DataTable rows = db.ReadDataTable("SELECT o.Slot,o.Amount,COALESCE(p.Progress,0) Progress FROM data_quest_objective o LEFT JOIN character_quest_progress p ON p.CharID=@c AND p.QuestID=o.QuestID AND p.Slot=o.Slot WHERE o.QuestID=@q AND o.Active=1 AND o.IsMob=1 AND o.HasToBeKilled=1",
                        new MySqlParameter("@c",c.ID),new MySqlParameter("@q",questId));
                    if (rows == null || rows.Rows.Count == 0) return false;
                    foreach (DataRow r in rows.Rows) if (Convert.ToUInt32(r["Progress"]) < Convert.ToUInt32(r["Amount"])) return false;
                    return true;
                }
            }
            catch { return false; }
        ...
        public static bool Complete(ZoneCharacter c, uint questId, uint selectedIndex = 0, bool selectionProvided = false)
        {
            ...
            try
            {
                using (DatabaseClient db = Program.DatabaseManager.GetClient())
                {
                    DataTable state = db.ReadDataTable("SELECT Status FROM character_quest_state WHERE CharID=@c AND QuestID=@q",new MySqlParameter("@c",c.ID),new MySqlParameter("@q",questId));
                    if (state == null || state.Rows.Count == 0 || Convert.ToByte(state.Rows[0]["Status"]) != PqsInProgress) return false;
                    byte completionStatus = IsRepeatable(db, questId) ? PqsSoon : PqsDone;
                    db.ExecuteQuery("UPDATE character_quest_state SET Status=@s WHERE CharID=@c AND QuestID=@q",new MySqlParameter("@s",completionStatus),new MySqlParameter("@c",c.ID),new MySqlParameter("@q",questId));
        ...
        private static bool HasSelectableRewards(ZoneCharacter c, uint questId, DatabaseClient db)
        {
            ...
            for (int off=0; off+12<=b.Length; off+=12) if (b[off+8] == 2) return true;
            return false;
        }

        private static bool IsRepeatable(DatabaseClient db, uint questId)
        {
            DataTable rows = db.ReadDataTable("SELECT Repeatable FROM data_quest WHERE QuestID=@q", new MySqlParameter("@q", questId));
            ...
            try
            {
                using (DatabaseClient db = Program.DatabaseManager.GetClient())
                {
                    ...
                    for (int off = 0; off + 12 <= b.Length; off += 12)
                    {
                        ...
                        if (ordinal == selectedIndex) return true;
                        ordinal++;
                    }
                }
            }
        ...
        private static byte[] Hex(string s)
        {
            ...
            byte[] r=new byte[s.Length/2]; for(int i=0;i<r.Length;i++) r[i]=Convert.ToByte(s.Substring(i*2,2),16); return r;
        }
    }
}
