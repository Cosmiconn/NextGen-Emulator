...
namespace NextGen.Zone.Handlers
{
    ...
    {
        private sealed class DialogSession { public uint DialogID; public ushort Seq; public QuestScriptMachine Machine; }
        ...
        private static void SendDialogPage(ZoneClient client, uint dialogId, QuestScriptMachine machine)
        {
            ...
            DialogSession session;
            lock(Sync){DialogSession previous;Sessions.TryGetValue(client.Character.ID,out previous);ushort seq=previous==null?(ushort)1:(ushort)(previous.Seq+1);session=new DialogSession{DialogID=dialogId,Seq=seq,Machine=machine};Sessions[client.Character.ID]=session;}
