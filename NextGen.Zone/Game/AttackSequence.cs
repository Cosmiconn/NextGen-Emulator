        ...
        public void Update(DateTime now)
        {
                ...
                if (IsAoE)
                {
                    ...
                    else if (this.State == AnimationState.AoEDo)
                    {
                        ...
                        foreach (var v in attacker.Map.GetObjectsBySectors(attacker.MapSector.SurroundingSectors))
                        {
                            ...
                            else if (v is Mob && attacker is ZoneCharacter)
                            {
                                ...
                                QuestRuntime.RecordMobKill(attacker as ZoneCharacter, (v as Mob).ID);
                            }
                        ...
                        foreach (var v in victims) if (v.HPLeft == 0) Handler9.SendDieAnimation(attacker, v.MapObjectID);
                        victims.Clear();
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (victim.GetIgnorePhysicalDamagePercent() > 0 && Program.Randomizer.Next(0, 100) < victim.GetIgnorePhysicalDamagePercent()) dmg = 0;
                    if (dmg > victim.HP) victim.HP = 0;
                    ...
                    if (victim.IsDead)
                    {
                        if (victim is Mob && attacker is ZoneCharacter)
                        {
                            ...
                            (attacker as ZoneCharacter).GiveMobKillTitleProgress();
                            QuestRuntime.RecordMobKill(attacker as ZoneCharacter, (victim as Mob).ID);
                        }
