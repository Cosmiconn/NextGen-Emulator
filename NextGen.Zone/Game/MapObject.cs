...
namespace NextGen.Zone.Game
{
    public abstract class MapObject
    {
        ...
        public int GetCriticalRateBuff() { return Buffs.CriticalRate; }
        public int GetDropRateBonusPercent() { return Buffs.DropRatePercent; }
        public int GetCastingTimeBonusPercent() { return Buffs.CastingTimeBonusPercent; }
        public int GetIgnoreMagicDamagePercent() { return Buffs.IgnoreMagicDamagePercent; }
        public int GetIgnorePhysicalDamagePercent() { return Buffs.IgnorePhysicalDamagePercent; }
        public int GetAttackSpeedBuff() { return Buffs.AttackSpeed; }
        public int GetMoveSpeedBuff() { return Buffs.MoveSpeed; }
        public int GetDotDamageBonusPercent(AbStateInfo debuff)
        {
            int bonus = Buffs.DotDamageBonusPercent;
            if (debuff.DispelIndex == 4) bonus += Buffs.PoisonDamageBonusPercent;
            if (debuff.SubDispelIndex == 4) bonus += Buffs.BloodingDamageBonusPercent;
            return bonus;
        }
        public void AddPassiveSkill(FiestaLib.Data.PassiveSkillInfo skill) { Buffs.AddPassiveSkill(skill); }
        public void RemovePassiveSkill(FiestaLib.Data.PassiveSkillInfo skill) { Buffs.RemovePassiveSkill(skill); }
        public void ApplySkillAbStates(ActiveSkillInfo skillInfo, MapObject buffRecipient, MapObject debuffRecipient)
        {
            ...
            {
                if (slot.SuccessRate < 100 && Program.Randomizer.Next(0, 100) >= slot.SuccessRate) continue;
        ...
        public void ForceMove(MapObject source, int distance, bool towardSource)
        {
            ...
            using (var packet = Handler8.MoveObject(this, oldPos.X, oldPos.Y, false)) { MapSector.Broadcast(packet); }
        }
        ...
        {
            if (totally) { HP = MaxHP; SP = MaxSP; }
            else if (Buffs.ReviveHealRatePermille > 0) HP = (uint)(MaxHP * (uint)Math.Min(1000, Buffs.ReviveHealRatePermille) / 1000);
            else HP = 50;
        }
        public virtual void Damage(MapObject bully, uint amount, bool isSP = false, bool isReflected = false)
        {
            if (!isReflected && !isSP)
            {
                if (Buffs.MissRatePercent > 0 && Program.Randomizer.Next(0, 100) < Buffs.MissRatePercent) return;
            ...
            if (isSP) { if (SP < amount) SP = 0; else SP -= amount; }
            else { if (HP < amount) HP = 0; else HP -= amount; if (Buffs.MinHP > 0 && HP < (uint)Buffs.MinHP) HP = (uint)Buffs.MinHP; }
            if (!isReflected && !isSP && bully != null && Buffs.ReflectDamagePercent > 0)
            {
                uint reflected = (uint)(amount * Buffs.ReflectDamagePercent / 100);
                if (reflected > 0) bully.Damage(this, reflected, false, true);
            }
            if (bully == null)
            {
                if (this is ZoneCharacter)
                {
                    ...
                    if (isSP) Handler9.SendUpdateSP(character); else Handler9.SendUpdateHP(character);
                }
            }
            else
            {
                if (this is Mob && ((Mob)this).AttackingSequence == null) this.Attack(bully);
                else if (this is ZoneCharacter && !((ZoneCharacter)this).IsAttacking) this.Attack(bully);
            }
        }
        public virtual void Heal(uint amount, bool isSP = false)
        {
            if (isSP) { if (SP + amount > MaxSP) SP = MaxSP; else SP += amount; }
            else { if (HP + amount > MaxHP) HP = MaxHP; else HP += amount; }
            if (this is ZoneCharacter character) { if (isSP) Handler9.SendUpdateSP(character); else Handler9.SendUpdateHP(character); }
        }
        public abstract void Update(DateTime date);
        public abstract Packet Spawn();
        #endregion
        #region Event-Stuff
        protected virtual void OnHpSpChanged() { if (HpSpChanged != null) HpSpChanged(this, new EventArgs()); }
