...
## Quest event signatures

The original public CodeView symbol for level changes is:

`?Occure_LevelChange@CQuest@@UAEXGEE@Z`

This means `nQuestID` is a WORD, followed by `nPlayerLevel` and `nDoneLevel` as BYTE values. The exact optimized Zone.exe address mapping for this routine remains unresolved; the earlier `GGG` signature was incorrect and is superseded by the symbol above.
...
This is a structure mapping only. It does **not** by itself establish how these source fields map to the emulator's normalized SQL columns, nor does it establish the complete higher-level meaning of every branch in `GetQuestStatusWithNPC`.

No unsupported `GGG` LevelChange signature should be used.
